...
class MetricsCollector:
    ...
    def record_cache_hit(self, hit: bool, cache_type: str = "general"):
        """Enregistre un hit/miss de cache avec type spécifique"""
        labels = {"cache_type": cache_type}
        if hit:
            self.increment_counter("cache_hits", labels=labels)
        else:
            self.increment_counter("cache_misses", labels=labels)
        
        # Mise à jour des métriques RAG pour compatibilité
        with self._lock:
            if hit:
    ...
    def record_search_performance(self, search_type: str, duration: float, results_count: int):
        """Enregistre les performances de recherche"""
        labels = {"search_type": search_type}
        self.record_timer(f"search_duration_{search_type}", duration, labels)
        self.set_gauge(f"search_results_{search_type}", results_count, labels)
    
    def record_embedding_performance(self, embedding_type: str, duration: float, quantized: bool = False):
        """Enregistre les performances d'embedding"""
        labels = {"embedding_type": embedding_type, "quantized": str(quantized)}
        self.record_timer("embedding_duration", duration, labels)
        if quantized:
            self.increment_counter("embeddings_quantized", labels=labels)
    
    def record_chromadb_performance(self, operation: str, duration: float, batch_size: int = 1):
        """Enregistre les performances ChromaDB"""
        labels = {"operation": operation}
        self.record_timer("chromadb_duration", duration, labels)
        self.set_gauge("chromadb_batch_size", batch_size, labels)
