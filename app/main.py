...
# Variable globale pour stocker le processus du bot Telegram
    ...
    def start_telegram_bot():
        ...
        try:
            ...
            telegram_bot_process = subprocess.Popen(
                ...
                cwd=str(bot_cwd),
                env=env,
                start_new_session=False  # Changé pour permettre l'arrêt propre
            )
...
async def shutdown_event():
    """Événements d'arrêt pour nettoyage propre"""
    from app.utils.logging import logger
    global telegram_bot_process
    
    logger.info("Arrêt du serveur en cours...")
    
    # Arrêter le bot Telegram s'il est en cours d'exécution
    if telegram_bot_process and telegram_bot_process.poll() is None:
        try:
            ...
            if telegram_bot_process.poll() is None:
                logger.warning("Forçage de l'arrêt du bot Telegram")
                telegram_bot_process.kill()
                
            logger.info("✅ Bot Telegram arrêté")
            
        except Exception as e:
            logger.error(f"Erreur lors de l'arrêt du bot Telegram: {e}")
